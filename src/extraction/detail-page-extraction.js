...
const extractDetailedRoomsInfo = () => {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        const dataRoomId = row.find("select").attr("data-room-id") ?? "";
        let dataBlockId = (
            row.find("select").attr("data-block-id") ?? ""
        ).replaceAll(dataRoomId, "");
        if (dataBlockId !== "" && dataBlockId[0] !== "_")
            dataBlockId = "_" + dataBlockId;
        const roomId = dataRoomId + dataBlockId;
        if (roomRow.length > 0) {
